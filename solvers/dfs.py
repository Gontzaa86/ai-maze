...
@register_solver(
    name="dfs",
    display_name="DFS",
    description="Busca una solución mediante Depth-First Search.",
    category="classic"
)
class DFSSolver(Solver):
    def solve(self, maze: Maze, start: tuple[int, int], end: tuple[int, int]) -> SolveResult:
        ...
        while stack:
            ...
            neighbors = self._get_neighbors(maze, current)
    ...
    def _start_event(self, position: tuple[int, int]) -> SolverEvent:
        return SolverEvent(type = EventType.START, position=position)

    def _move_event(self, current: tuple[int, int], neighbor: tuple[int, int]) -> SolverEvent:
        return SolverEvent(
            type=EventType.MOVE,
            position=neighbor,
    ...
    def _solved_event(self, position: tuple[int, int]) -> SolverEvent:
